...
Deliberately not exposed yet: boxes
-----------------------------------
Listing a box's tracks would be the most O2M-specific thing here, and it is
missing on purpose. `/api/box` *activates* a box (it is an action), and
`/api/box_info` returns only its definition. There is no read-only endpoint that
resolves a box into URIs, because `tracklistfill_auto` adds to the tracklist as
it goes rather than returning a list — resolve and mutate are entangled.

Exposing boxes needs that separated in the service first, where the logic
belongs. Doing it here would mean reimplementing box semantics in the extension,
which is exactly what this extension must not do.
"""
