...
class DatabaseHandler():
    ...
    def get_uris_podcasts_notread(self, limit=15):
        ...
        #pattern="%podcast+%"
        date_now = datetime.datetime.utcnow().timestamp()
        query = Stats.select().where( ((Stats.uri % '%podcast+%') | (Stats.uri % '%youtube:video%')| (Stats.uri % '%yt:%'))& (Stats.read_end <= 0.9)& (Stats.read_position > 30000)& (Stats.option_type != "info")).order_by(Stats.last_read_date.desc()).limit(limit)
        results = self.transform_query_to_list(query)
        print (results)
        if len(results) > 0:
            uris = [o.uri for o in results]
